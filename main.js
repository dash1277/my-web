...
if (buyButton) {
    buyButton.addEventListener("click", () => {
        ...
        document.querySelectorAll(".cart-box").forEach(cartBox => {
            let name = cartBox.querySelector(".cart-product-title").textContent;
...
document.getElementById("add-favourites").addEventListener("click", saveFavourites);

function saveFavourites() {
    const inputs = Array.from(document.querySelectorAll('input[type="number"]')).map(input => ({
      name: input.getAttribute("product-title"),
      quantity: input.value,
      price: input.getAttribute("price")
    }));
    localStorage.setItem("favourites", JSON.stringify(inputs));
    alert("Items added as favorites!");
}

//Apply Favorite Button
document.querySelector('.btn-apply#apply-favourites').addEventListener('click', applyFavourites);
...
function applyFavourites() {
  ...
  favourites.forEach(item => {
    ...
    cartBox.innerHTML = `<img src="${productImg}" alt="" class="cart-img">
                    <div class="detail-box">
                        <div class="cart-product-title">${title}</div>
                        <div class="cart-price">${price}</div>
                        <input type="number" name="" id="" value="1" class="cart-quantity">
